class StrategyManager {
  ...
  evaluateMomentum(liveBar, prevBar, obi, atr) {
    ...
    const minExpansion = Math.max(1.3, this.config.Momentum?.MomMultiplier || 1.3);
    ...
    if (obi < -0.2 && direction === 'BUY') score *= 0.4;
    if (obi > 0.2 && direction === 'SELL') score *= 0.4;
  ...
  evaluateTrap(liveBar, localRangeHigh, localRangeLow, obi, atr) {
    ...
    const wickRatio = this.config.Trap?.TrapWickRatio || 0.55;
  ...
  evaluateWickThief(liveBar, currentATR, volumeDelta, obi) {
    ...
    const wickMult = this.config.WickThief?.WT_WickMultiplier || 0.5;
    ...
    const bodyRatio = Math.abs(liveBar.close - liveBar.open) / barRange;
    if (bodyRatio > 0.45) return { signal: false };
  ...
  evaluateCompression(barsCluster, currentAsk, currentBid, coilHigh, coilLow, obi, atr) {
    ...
    const compRatio = this.config.Compression?.CompRatio || 1.3;
  ...
  evaluateFlowReversal(liveBar, prevBar, obi, atr) {
    ...
    if (deltaShift > atr * 0.3 && priceShift > 0 && obi > 0.2 && bodyRatio > 0.3) {
